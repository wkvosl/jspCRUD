	...
	<div id="all_body_div">
		...
		<table id="newWriteTable">
			...
			<tr>
				<th>첨부파일</th>
				<td id="newWriteTable_td">
					<input type="file" id="input_file" name="realfilename" accept="image/*" class="form-control"> <span id="span"></span>
					...
					<c:if test="${not empty detail.realfilename }">
						<button type="submit" formmethod="post" formaction="fdel.do" form="modiForm"  class="btn btn-secondary btn-sm"
								onclick="return confirm('업로드 파일을 삭제하시겠습니까? 영구삭제 됩니다.');" id="uploadDel_btn">  <!-- alert msg못받음 -->
							삭제
						</button>
					</c:if>
